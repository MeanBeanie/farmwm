...
int main(int, char *[])
{
    ...
    if (!compositor.start())
    {
		...
		/* Loads the config file located at HOME/.config/farmwm/config.toml.
		 * Then goes through and launches all the programs stated in the "on-start" array */
		conf.initialize();

		for(std::string str : conf.onStart){
			LLauncher::launch(str);
		}
